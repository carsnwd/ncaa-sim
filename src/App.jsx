...
function initializeTeams(previousTeams = null) { const teams = {}, conferences = {}; Object.entries(CONFERENCES_DATA).forEach(([cn, conf]) => { conferences[cn] = []; conf.teams.forEach(t => { const priorElo = previousTeams?.[t.name]?.elo; const seasonStartElo = Math.min(priorElo ?? t.elo, 2000); teams[t.name] = { name: t.name, conference: cn, elo: seasonStartElo, seasonStartElo, startingElo: seasonStartElo, wins: 0, losses: 0, confWins: 0, confLosses: 0, streak: 0, streakType: null, lastOpponent: null, lastScore: null, lastResult: null }; conferences[cn].push(t.name); }); }); return { teams, conferences }; }
...
export default function NCAASimulator() {
  ...
  return (
    <div style={S.container}>
      ...
      {currentWeek < schedule.length && <div style={S.simBar}><button style={S.simButton} onClick={simulateWeek}>▶ SIMULATE WEEK {currentWeek + 1}: {schedule[currentWeek].phase}</button><button style={S.simButtonSmall} onClick={simulateToBracketStart}>🏀 To Bracket</button><button style={S.simButtonSmall} onClick={simulateToEndOfSeason}>🏁 End Season</button></div>}
      ...
      <div style={S.content}>
        ...
        {view === "top25" && <div style={S.card}><h3 style={S.cardTitle}>🏆 TOP 25 RANKINGS</h3><div style={S.rankHeader}><span style={{ width: 28 }}>#</span><span style={{ width: 26 }}></span><span style={{ flex: 1 }}>Team</span><span style={{ width: 45 }}>Conf</span><span style={{ width: 46, textAlign: "center" }}>Rec</span><span style={{ flex: 1.2 }}>Stk / Last</span><span style={{ width: 50, textAlign: "right" }}>Start</span><span style={{ width: 50, textAlign: "right" }}>Curr</span><span style={{ width: 40, textAlign: "right" }}>Rk</span></div>
          {top25.map((t, i) => {
...
const S = {
  ...
  content: { padding: 14, maxWidth: 880, width: "100%", boxSizing: "border-box", margin: "0 auto" },
  conferenceContent: { padding: 14, maxWidth: "none", width: "100%", boxSizing: "border-box", overflowX: "hidden" },
  ...
  powerConfGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 12, width: "100%", justifyContent: "stretch", alignItems: "start" },
